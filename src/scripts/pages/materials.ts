...
// function createPDFThumbnail(
//     filePath: string,
//     callback: (s: string, n: number) => void,
...
async function showMaterialTooltips() {
     ...
     */
    function wrapHtml(baseString: string, wrapperTag: string, wrapperProps?: Record<string, any>) {
    ...
    type GradeLoadHook = (el: Record<string, any>) => void;

    let gradeLoadHooks: GradeLoadHook[] = [];
    let userId;
    let immediateGradeLoadInvoke = false;
    let loadedGradeContainer: Record<string, any> = {};

    function registerGradeLoadHook(loadHook: GradeLoadHook) {
    ...
    // tr.type-document
    // async function processDocumentElement(value: HTMLElement) {
    ...
    //     $(value)
    //         .find(".attachments-file-name>a")
    //         .tipsy({ gravity: "w", html: true, title: () => loadedTextHolder.dataset.tooltipHtml });
    ...
    //     let documentInfoFromApi = await fetchApiJson(
    //         `/sections/${classId}/documents/${materialId}`
    //     );

    //     if (
    //         !documentInfoFromApi.attachments.files ||
    //         !documentInfoFromApi.attachments.files.file[0]
    //     ) {
    ...
    //     if (
    //         fileData.converted_download_path &&
    //         (fileData.converted_filemime == "application/pdf" ||
    //             new URL(fileData.converted_download_path).pathname.endsWith(".pdf"))
    //     ) {
    ...
    //         documentUrlFromApi = (
    //             await fetchWithApiAuthentication(fileData.converted_download_path)
    //         ).url;
    //     } else {
    //         value.dataset.schoologyPlusProcessedTooltip = "true";
    //         return;
    ...
    // assignment tooltips
    // tr.type-assignment
    function processAssignmentElement(value: HTMLElement) {
        let loadedGrade = "Loading...";
        $(value)
            .find(".item-title>a")
            .tipsy({ gravity: "n", html: true, title: () => loadedGrade });

        let assignmentId = value.id.match(/\d+/)![0];

        registerGradeLoadHook(function (grades: Record<string, any>) {
            let innerContent = "";
            ...
            if (!grades.grades[assignmentId]) {
                ...
                if (assignment.exception == 1) {
                    ...
                    let gradeElem = wrapHtml(
                        escapeForHtml(
                            (!assignment.max_points && assignment.grade >= 0 ? "+" : "") +
                                assignment.grade
                        ),
                        "span",
                ...
                if (grades.gradeDrops[assignmentId]) {
                    // assignment dropped
                    innerContent =
                        wrapHtml("Dropped", "span", { class: "tooltip-dropped-indicator" }) +
                        " " +
                        wrapHtml(innerContent, "span", { class: "tooltip-dropped-gradeinfo" });
                }
                ...
                if (assignment.category_id && grades.categories[assignment.category_id]) {
                    footerElements.push(
                        wrapHtml(
                            escapeForHtml(grades.categories[assignment.category_id].title),
                            "span",
                ...
                if (grades.dropboxes[assignmentId]) {
                    let validSubmissions = grades.dropboxes[assignmentId].filter(
                        (x: Record<string, any>) => !x.draft
                    );
                    if (validSubmissions.length == 0) {
                        footerElements.push(
                            wrapHtml("Not Submitted", "span", { class: "tooltip-not-submitted" })
                        );
                    } else {
                ...
                if (footerElements.length > 0) {
                    innerContent += wrapHtml(
                        footerElements.join('<span class="tooltip-horiz-divider"> | </span>'),
                        "p",
    ...
    // for (let docElem of courseProfileMaterials.querySelectorAll<HTMLElement>("tr.type-document")) {
    //     processDocumentElement(docElem);
    // }

    for (let assignElem of courseProfileMaterials.querySelectorAll<HTMLElement>(
        "tr.type-assignment"
    )) {
    ...
    let materialsMutationObserver = new MutationObserver(function (mutationRecords) {
        ...
        for (let addedNode of courseProfileMaterials.querySelectorAll<HTMLElement>(
            "tr.type-assignment, tr.type-document"
        )) {
    ...
    if (thisClassGrades) {
        ...
        for (let assignment of thisClassGrades.period.reduce(
            (accum: { assignment: any }[], curr: { assignment: any }) =>
    ...
    let missingAssignmentIds = Object.keys(loadedGradeContainer.grades)
        .filter(x => ourAssignments.assignment.findIndex((y: Record<string, any>) => y.id == x) < 0)
        .filter(x => document.getElementById("n-" + x));
    ...
    if (missingAssignmentCt > 0) {
        Logger.log(
            `Fetched ${missingAssignmentCt} assignment(s) (${missingAssignmentErrorCt} error(s)) missing from summary API call`
        );
    }
    ...
    for (let assignment of ourAssignments.assignment) {
        ...
        if (
            +assignment.allow_dropbox &&
            (!gradeInfo || gradeInfo.grade === null || gradeInfo.exception)
        ) {
            // "dropboxes," or places to submit documents via Schoology
            // another API call
            loadedGradeContainer.dropboxes[assignment.id] = (
                await fetchApiJson(`/sections/${classId}/submissions/${assignment.id}/${userId}`)
            ).revision;
    ...
    if (containerDiv) {
        ...
        for (let gradeItemRow of containerDiv.querySelectorAll<HTMLElement>(".item-row")) {
            loadedGradeContainer.gradeDrops[gradeItemRow.dataset.id!.match(/\d+/)![0]] =
                gradeItemRow.classList.contains("dropped");
        }
    }
