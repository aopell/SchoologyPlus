...
export async function preload() {
    Logger.log(
        `Loaded Schoology Plus version ${chrome.runtime.getManifest().version}${
...
export function loadBetaFeatures() {
    let betaCode = FORCED_BETA_TEST || Setting.getValue<string>("beta");
...
async function attemptDisplayCourseAliases() {
    ...
    if (Setting.getValue("courseAliases")) {
        ...
        myClasses.push(
            ...(await Promise.all(
                Object.keys(Setting.getValue<Record<string, string>>("courseAliases", {}))
                    .filter(
                        aliasedCourseId =>
                            !myClasses.some((x: Record<string, string>) => x.id == aliasedCourseId)
                    )
                    .filter(aliasedCourseId =>
                        Setting.getNestedValue("courseAliases", aliasedCourseId)
        ...
        for (let aliasedCourseId in Setting.getValue<Record<string, string>>("courseAliases", {})) {
            ...
            sheet.insertRule(
                ...
                content: "${Setting.getNestedValue("courseAliases", aliasedCourseId)}";
                visibility: visible;
                word-spacing:normal;
        ...
        applyCourseAliases = function (mutationsList?: MutationRecord[]) {
            ...
            for (let jsonCourse of myClasses) {
                if (!jsonCourse || !Setting.getNestedValue("courseAliases", jsonCourse.id)) {
                    continue;
                }
                ...
                for (let findText of findTexts) {
                    ...
                    let currentCourseAlias = Setting.getNestedValue<string>(
                        "courseAliases",
                        jsonCourse.id
                    );
...
function autoBypassLinkRedirects() {
    if (
        ...
        Setting.getValue("autoBypassLinkRedirects") === "enabled"
    ) {
