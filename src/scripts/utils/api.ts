...
async function backgroundPageFetch(url: RequestInfo, init: RequestInit, bodyReadType: "json" | "text") {
    try {
        let response = await browser.runtime.sendMessage({ type: "fetch", url: url, params: init, bodyReadType: bodyReadType });

        if (response === undefined || response === null) {
            Logger.error("[backgroundPageFetch] Response is undefined or null", response, browser.runtime.lastError);
            throw new Error("Response is undefined or null. Last error: " + browser.runtime.lastError);
        }
 ...
 */
function createFetchRateLimitWrapper(requestsPerInterval: number, interval: number): (input: RequestInfo, init: RequestInit, bodyReadType: "json" | "text") => Promise<Response> {
    ...
    function rateLimitedFetch(this: any, input: RequestInfo, init: RequestInit, bodyReadType: "json" | "text"): Promise<Response> {
        if (callsThisCycle == 0) {
 ...
 */
async function fetchWithApiAuthentication(url: string, baseObj?: { [s: string]: string; }, useRateLimit: boolean = true, bodyReadType: "json" | "text" = "json"): Promise<Response> {
...
function getUserId(): number {
    try {
        const trackerFrame = document.querySelector<HTMLIFrameElement>("iframe[src*=session-tracker]");
        if (!trackerFrame) {
 ...
 */
function createApiAuthenticationHeaders(apiKeys: string[], baseObj?: { [s: string]: any; }): { [s: string]: string; } {
